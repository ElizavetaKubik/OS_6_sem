...
typedef bool(*Delete)(HT::HashTableData*, const char*);
...
int main(int argc, char* argv[])
{
	...
	if (argc == 2)
	{
		try
		{
			...
			Delete deleteF = (Delete)GetProcAddress((HMODULE)dll, "Delete");
			...
			while (true)
			{
				...
				try
				{
					success_result = deleteF(hash_table_data, delete_key.c_str());
				}
