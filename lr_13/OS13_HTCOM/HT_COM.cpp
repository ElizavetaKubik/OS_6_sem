...
STDMETHODIMP HT_COM::Create(HT::HashTableData** result, int inCapacity, int inSecSnapshotInterval, int inMaxKeyLength, int inMaxValueLength, string inFileName)
{
	*result = HT::CreateHT(inCapacity, inSecSnapshotInterval, inMaxKeyLength, inMaxValueLength, inFileName);
...
STDMETHODIMP HT_COM::Open(string inFileName, HT::HashTableData** result)
{
	SEQHT;
	LOGHT("Open file with name: ", inFileName);

	HT::HashTableData* temp = HT::OpenHT_FromFile(Utilities::GetModulePath() + inFileName);
	if (temp == NULL) 
	{
		temp = HT::CreateHT_FromFile((Utilities::GetModulePath() + inFileName).c_str());
