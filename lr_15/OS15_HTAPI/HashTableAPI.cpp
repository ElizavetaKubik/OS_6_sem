...
namespace HTAPI
{
	...
	HT::HashTableData* HTAPI::Create(int inCapacity, int   inSecSnapshotInterval, int inMaxKeyLength, int inMaxValueLength, string inFileName, LPCWSTR HTUsersGroup)
	{
		HT::HashTableData* pht;
 		if (SUCCEEDED(pIHashTable->Create(&pht, inCapacity, inSecSnapshotInterval, inMaxKeyLength, inMaxValueLength, inFileName, HTUsersGroup)))
		{
			return pht;
		}
		else
	...
	HT::HashTableData* HTAPI::CreateFromFile(string inFileName)
	{
		HT::HashTableData* pht;
		if (SUCCEEDED(pIHashTable->CreateFromFile(&pht, inFileName)))
		{
			return pht;
		}
		else
	...
	{
		HT::HashTableData* pht;
		if (SUCCEEDED(pIHashTable->Open(&pht, inFileName)))
		{
			return pht;
		}
		else
	...
	HT::HashTableData* HTAPI::Open(string inFileName, LPCWSTR userName, LPCWSTR password)
	{
		HT::HashTableData* pht;
		if (SUCCEEDED(pIHashTable->Open(&pht, inFileName, userName, password)))
		{
			return pht;
		}
		else
