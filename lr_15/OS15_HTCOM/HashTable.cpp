...
#pragma region InnerFunctions
	...
	HANDLE CreateMutex_HT(wchar_t* inMutexName)
	{
		SECURITY_ATTRIBUTES sa = GetSA();
		HANDLE mutex_handler = CreateMutex(&sa, FALSE, inMutexName);
...
#pragma region snap shots
	...
	bool Snap(HashTableData* inHashTableData)
	{
		...
		HANDLE mutex_handler = CreateMutex_HT(inHashTableData->MutexName);
...
#pragma endregion
	...
	HashTableData* CreateHT(int inCapacity, int inSecSnapshotInterval, int inMaxKeyLength, int inMaxValueLength, string inFileName, LPCWSTR HTUsersGroup)
	{		
		if (!Auth::checkGroupExisting(HTUsersGroup) || !Auth::checkCurrentUserGroup(HTUsersGroup) || !Auth::checkCurrentUserGroup(L"Administrators"))
		{
			return NULL;
		}
		...
		//file should have at least 1 symbol, cuz CreateFileMapping function needs it
		DWORD written;
		...
		general_prefix_name = L"Global\\" + general_prefix_name;
		...
		SECURITY_ATTRIBUTES sa = GetSA();
		HANDLE file_mapping_handler = CreateFileMapping(file_handler, &sa, PAGE_READWRITE, 0, ht_size, file_map_name.c_str());
	...
	HashTableData* CreateHT_FromFile(const char* inFileName, LPCWSTR HTUsersGroup)
	{
		if (!Auth::checkGroupExisting(HTUsersGroup) /* || !Auth::checkCurrentUserGroup(HTUsersGroup)*/)
		{
			return NULL;
		}
		...
		SECURITY_ATTRIBUTES sa = GetSA();
		HANDLE file_mapping_handler = CreateFileMapping(file_handler, &sa, PAGE_READWRITE, 0, 0, file_map_name.c_str());
		if (!file_mapping_handler)
		{
	...
	HashTableData* Open(string inFileName)
	{
		HashTableData* hashTable = OpenHT_FromFile(inFileName);
		if (hashTable != NULL)
		{
			if (Auth::checkCurrentUserGroup((LPCWSTR)hashTable->HTUsersGroup))
			{
				return hashTable;
			}
			else
	...
	HashTableData* Open(string inFileName, LPCWSTR userName, LPCWSTR password)
	{
		...
		if (hashTable != NULL)
		{		
			if (!Auth::checkUserInGroup(userName, (LPCWSTR)hashTable->HTUsersGroup))
			{
				Close(hashTable);
				return NULL;
			}
...
#pragma endregion
	...
	Element* Get(HashTableData* inHashTableData, const char* inElementKey)
	{
		if (inHashTableData == nullptr)
		{
			cout << "hash table isn't opened, operation is cancelled";
